...
#
# Sibling of token-report.sh on the same Stop event: separate state file, both
# honor stop_hook_active, both emit decision:block + stderr. They never share
# mutable state, so they block the same Stop independently; their reasons are
# additive (append the token report AND raise the boundary checkpoint). Run the
# pre-launch two-hook aggregation test before trusting that both reasons surface;
# if a hard reason is ever dropped, this hook re-fires next Stop, it does not go
# silent, because its cooldown is keyed to verified tool use, not to emission.
#
...
# matching token-report.sh. Our state only advances when we actually fire, and
# the checkpoint raised during that continuation is detected on the next FRESH
# Stop by timestamp, so nothing needs saving here.
...
# cache_read already reflects the whole re-sent prefix, so one message is the
# cumulative size; token-report.sh's reduce-sum is a per-turn spend delta, a
# different metric. The same pass also returns that message's model + timestamp
...
# past the post-ack cooldown. decision:block + stderr, mirroring token-report.sh.
