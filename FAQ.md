...
### Does anything override a tier's pinned model or effort even when route-gate.sh allows it?

Yes, two Claude Code environment variables, neither one of ours. `CLAUDE_CODE_SUBAGENT_MODEL` resolves before the per-invocation `model` parameter and before the subagent's own frontmatter, so it silently overrides even a tier agent's pinned model. `CLAUDE_CODE_EFFORT_LEVEL` does the same thing for `effort`: Claude Code's own docs are explicit that frontmatter effort "override[s] the session level but not the environment variable." `route-gate.sh` can't stop either, neither is a decision the hook gets consulted on, but it detects both and logs accordingly instead of a plain `allow-tier`: `allow-tier-model-overridden`, `allow-tier-effort-overridden`, or `allow-tier-model-and-effort-overridden` if both are set, recording the actual overriding value(s) instead of pretending the frontmatter ran unmodified.

Treat those log entries as observed, not verified: the hook only sees its own copy of the environment. If the model override value is excluded by an organization's `availableModels` allowlist, Claude Code silently skips it and falls back to the inherited model instead, and the hook has no way to know that happened, so the logged value can be wrong in that specific case. Check `/routing-status` if a tier's cost or behavior looks wrong regardless, these two variables are the first thing to rule out. Setting `CLAUDE_CODE_SUBAGENT_MODEL` to `inherit`, or `CLAUDE_CODE_EFFORT_LEVEL` to `auto`, is a no-op, same as leaving either unset.

### Can an organization silently cap effort below what a tier asks for?

Yes, and this one `route-gate.sh` cannot detect at all, there is no local environment variable or file for it to check. Claude Enterprise admins can set a maximum effort level per model per role. Ask for a level above the cap and Claude Code runs at the cap instead: in interactive sessions and plain-text `--print` runs this shows a warning naming both levels, but in background agents the clamp is silent, no warning at all. Subagents run in the background by default since Claude Code v2.1.198, so on an Enterprise account with a role-level effort cap, `hard`'s `xhigh` or `super`'s `max` can get silently capped with zero signal anywhere, not in the hook's log, not in the subagent's own output. If tiers seem to be reasoning less than their pinned `effort` should produce and nothing in `/routing-status` explains it, ask your org admin whether a role-level effort cap applies to the models these tiers use.
