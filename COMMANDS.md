...
Run this in a chat that's been open since before you last edited CLAUDE.md or the routing skill, so it picks up the current rules without a restart. It re-reads both files and returns two checkable artifacts (the current routing iron rule, the current `context-check.sh` thresholds) so you can confirm it took effect instead of trusting a claim.
